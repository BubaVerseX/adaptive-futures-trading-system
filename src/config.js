...
function loadConfig() {
  ...
  const config = {
    ...
    minSignalScore: numberValue("MIN_SIGNAL_SCORE", 48, { positive: true, maximum: 100 }),
    ...
    minVolumeSpike: numberValue("MIN_VOLUME_SPIKE", 1.6, { positive: true }),
    minBurstMomentumPct: numberValue("MIN_BURST_MOMENTUM_PCT", 0.12, { positive: true }),
    fomoMomentumPct: numberValue("FOMO_MOMENTUM_PCT", 0.22, { positive: true }),
    minMomentumPersistenceCandles: numberValue("MIN_MOMENTUM_PERSISTENCE_CANDLES", 3, { positive: true, integer: true, maximum: 6 }),
    ...
    minProjectedEdgePct: numberValue("MIN_PROJECTED_EDGE_PCT", 0.65, { minimum: 0 }),
    minExpectedMovePct: numberValue("MIN_EXPECTED_MOVE_PCT", 1.1, { minimum: 0 }),
    expectedMoveAtrMultiplier: numberValue("EXPECTED_MOVE_ATR_MULTIPLIER", 1.25, { positive: true }),
    minEdgeToCostRatio: numberValue("MIN_EDGE_TO_COST_RATIO", 2.1, { positive: true }),
    minConvictionScore: numberValue("MIN_CONVICTION_SCORE", 58, { positive: true, maximum: 100 }),
    ...
    choppyMarketPenalty: numberValue("CHOPPY_MARKET_PENALTY", 16, { minimum: 0, maximum: 30 }),
    ...
    maxChopScore: numberValue("MAX_CHOP_SCORE", 2, { minimum: 0, maximum: 10 }),
    minRangeExpansion: numberValue("MIN_RANGE_EXPANSION", 0.85, { positive: true }),
    minDirectionalBodyStrength: numberValue("MIN_DIRECTIONAL_BODY_STRENGTH", 0.5, { positive: true, maximum: 1 }),
    ...
    explorationTradeRatio: numberValue("EXPLORATION_TRADE_RATIO", 0.25, { minimum: 0, maximum: 0.5 }),
    explorationMinSignalScore: numberValue("EXPLORATION_MIN_SIGNAL_SCORE", 40, { positive: true, maximum: 100 }),
    explorationMinConvictionScore: numberValue("EXPLORATION_MIN_CONVICTION_SCORE", 48, { positive: true, maximum: 100 }),
    explorationMinProjectedEdgePct: numberValue("EXPLORATION_MIN_PROJECTED_EDGE_PCT", 0.28, { minimum: 0 }),
    explorationMinEdgeToCostRatio: numberValue("EXPLORATION_MIN_EDGE_TO_COST_RATIO", 1.45, { positive: true }),
    explorationRiskMultiplier: numberValue("EXPLORATION_RISK_MULTIPLIER", 0.35, { positive: true, maximum: 1 }),
    explorationMaxChopScore: numberValue("EXPLORATION_MAX_CHOP_SCORE", 3, { minimum: 0, maximum: 10 }),
    explorationMaxTradesPerDay: numberValue("EXPLORATION_MAX_TRADES_PER_DAY", 8, { minimum: 0, integer: true }),
    adaptiveLearningEnabled: booleanValue("ADAPTIVE_LEARNING_ENABLED", true),
    minAdaptiveTrades: numberValue("MIN_ADAPTIVE_TRADES", 6, { positive: true, integer: true }),
    minAdaptiveBucketTrades: numberValue("MIN_ADAPTIVE_BUCKET_TRADES", 3, { positive: true, integer: true }),
    ...
    aggressiveWinRatePct: numberValue("AGGRESSIVE_WIN_RATE_PCT", 58, { positive: true, maximum: 100 }),
    adaptiveRecoveryWinRatePct: numberValue("ADAPTIVE_RECOVERY_WIN_RATE_PCT", 45, { positive: true, maximum: 100 }),
    adaptiveRecoveryLookbackTrades: numberValue("ADAPTIVE_RECOVERY_LOOKBACK_TRADES", 8, { positive: true, integer: true }),
    ...
    adaptiveBlacklistMinutes: numberValue("ADAPTIVE_BLACKLIST_MINUTES", 30, { minimum: 0 }),
