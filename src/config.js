...
function loadConfig() {
  ...
  const config = {
    ...
    microThresholdSweepBps: String(process.env.MICRO_THRESHOLD_SWEEP_BPS || "0.1,0.25,0.5,1,1.5,2,3,5,8,10,15,20")
      ...
      .map((value) => Number(value.trim()))
      .filter((value) => Number.isFinite(value) && value >= 0),
    microValidationMaxDrawdown: numberValue("MICRO_VALIDATION_MAX_DRAWDOWN", 5, { minimum: 0 }),
    ...
    v23PromotionOptimizationMode: booleanValue("V23_PROMOTION_OPTIMIZATION_MODE", false),
    v23PromotionReportFile: process.env.V23_PROMOTION_REPORT_FILE || path.join(PROJECT_ROOT, "data", "research", "reports", "promotion-v23.json"),
    v23LiveProfileFile: process.env.V23_LIVE_PROFILE_FILE || path.join(PROJECT_ROOT, "data", "research", "reports", "live-profile-v23.json"),
    v23MinProfitFactor: numberValue("V23_MIN_PROFIT_FACTOR", 1.5, { minimum: 0 }),
    v23MaxDrawdownUsdt: numberValue("V23_MAX_DRAWDOWN_USDT", 85, { minimum: 0 }),
    v23MinTradeCount: numberValue("V23_MIN_TRADE_COUNT", 12, { minimum: 0, integer: true }),
